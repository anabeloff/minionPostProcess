...
rule all:
    input:
       ...
       porechop_dir = expand("{res}/porechop_out", res = directory(config["resdir"])),
       trimmed_dir = expand("{res}/fastq_trimmed", res = directory(config["resdir"]))
...
rule porechop_trim:
    input:
       fastq_merged = expand("{res}/merged.fastq", res = directory(config["resdir"]))
    output:
       porechop_dir = temp(directory(expand("{res}/porechop_out", res = directory(config["resdir"])))),
       trimmed_dir = directory(expand("{res}/fastq_trimmed", res = directory(config["resdir"])))
    conda: "envs/deepbinner_env.yml"
    params:
    ...
    threads: config["threads"]
    shell: """
       echo "STEP 4.1 - Porechop run with two barcodes option."
       ...
       echo "STEP 4.2 - Porechop run for binned FASTQ files."
